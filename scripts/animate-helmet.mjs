...
for(let y=0;y<size;y++)for(let x=0;x<size;x++){
 ...
 const wind=Math.pow(Math.max(0,Math.min(1,(root-y)/135)),1.5);
...
try{
 for(let f=0;f<frames;f++){
  ...
  for(const p of points){
   ...
   const gust=13*Math.sin(phase)+6*Math.sin(phase*2-p.x*.013)+3*Math.sin(phase*3+p.y*.04);
   const x=Math.round(p.x+p.wind*gust);
   const y=Math.round(p.y-p.wind*(3*Math.sin(phase)+2*Math.sin(phase*2+p.x*.014)));
  ...
  if(f===0){const rgba=Buffer.alloc(size*size*4);for(let i=0;i<size*size;i++){rgba[i*4]=rgba[i*4+1]=rgba[i*4+2]=255;rgba[i*4+3]=pixels[i*3];}await sharp(rgba,{raw:{width:size,height:size,channels:4}}).webp({quality:94,alphaQuality:100}).toFile('public/animation/helmet-wind-poster.webp');}
 }
 const run=args=>execFileSync(ffmpeg,['-hide_banner','-loglevel','error','-y',...args]);
 run(['-framerate','12','-i',`${scratch}/%04d.png`,'-an','-c:v','libx264','-preset','slow','-crf','20','-pix_fmt','yuv420p','-movflags','+faststart','public/animation/helmet-wind.mp4']);
 run(['-i','public/animation/helmet-wind.mp4','-vf','scale=384:384:flags=lanczos','-an','-c:v','libx264','-preset','slow','-crf','22','-pix_fmt','yuv420p','-movflags','+faststart','public/animation/helmet-wind-small.mp4']);
