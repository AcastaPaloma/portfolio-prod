...
try {
  ...
  for (let frame = 0; frame < count; frame++) {
    ...
    if (frame === 0) {
      await rendered.clone().webp({ quality: 94 }).toFile(`${output}/minifigure-poster.webp`);
      await sharp(Buffer.from(svg.replace('<rect width="100%" height="100%" fill="black"/>',''))).webp({quality:94,alphaQuality:100}).toFile(`${output}/minifigure-transparent.webp`);
    }
  }
