...
import type { GymWorkout } from "@/lib/gym";
...
const TRAINING_SERIES = [
  { id: "flat-bench", label: "Flat bench", values: [155, 165, 175, 185, 190, 195] },
  { id: "incline-bench", label: "Incline", values: [115, 125, 130, 140, 145, 150] },
  { id: "smith-bench", label: "Smith flat", values: [135, 145, 155, 165, 170, 175] },
  { id: "smith-incline", label: "Smith incline", values: [105, 115, 125, 135, 140, 145] },
  { id: "smith-curl", label: "Smith curl", values: [45, 50, 55, 60, 65, 70] },
  { id: "tricep-extension", label: "Tricep", values: [35, 40, 45, 50, 55, 60] },
  { id: "seated-rows", label: "Seated rows", values: [90, 100, 105, 110, 115, 120] },
  { id: "lat-pulldown", label: "Lat pulldown", values: [100, 110, 115, 120, 125, 130] },
  { id: "chest-fly", label: "Chest fly", values: [60, 65, 70, 75, 80, 85] },
] as const;

type TrainingSeries = (typeof TRAINING_SERIES)[number];

function TrainingPlot({ workouts }: { workouts: GymWorkout[] | null }) {
  const [activeExercise, setActiveExercise] = useState<TrainingSeries["id"]>("flat-bench");
  const series = TRAINING_SERIES.find((item) => item.id === activeExercise) ?? TRAINING_SERIES[0];
  const loggedValues = workouts?.filter((workout) => workout.exercise.toLowerCase() === series.label.replace("Flat bench", "Flat bench press").replace("Incline", "Incline bench press").replace("Smith flat", "Smith bench press").replace("Smith incline", "Smith incline bench press").replace("Smith curl", "Smith bicep curl").replace("Tricep", "Tricep extension").toLowerCase()).sort((left, right) => left.workoutDate.localeCompare(right.workoutDate)).map((workout) => workout.weightLbs) ?? [];
  const values = loggedValues.length >= 2 ? loggedValues : series.values;
  const isLogged = loggedValues.length >= 2;
  const min = Math.min(...values) - 10;
  const max = Math.max(...values) + 10;
  const width = 250;
  const height = 92;
  const pointAt = (value: number, index: number) => ({ x: 8 + (index / (values.length - 1)) * (width - 16), y: 8 + ((max - value) / (max - min)) * (height - 20) });
  ...
  return (
    <article className="technical-card technical-card--plot" aria-labelledby="training-plot-title">
      <div className="technical-card-heading"><h2 id="training-plot-title">Load, over time</h2><a href="/gym">Open log</a></div>
      <div className="training-plot-frame">
        <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${series.label} ${isLogged ? "logged" : "estimated"} working weight increasing from ${values[0]} to ${values.at(-1)} pounds`}>
          <path className="training-plot-grid" d="M 8 18 H 242 M 8 48 H 242 M 8 78 H 242" />
          <motion.path className="training-plot-line" d={linePath} initial={false} animate={{ d: linePath }} transition={{ duration: 0.48, ease: AUTHORED_EASE }} />
          {values.map((value, index) => { const point = pointAt(value, index); return <motion.circle key={`${series.id}-${value}-${index}`} className="training-plot-dot" cx={point.x} cy={point.y} r="2.9" initial={false} animate={{ cx: point.x, cy: point.y }} transition={{ duration: 0.48, ease: AUTHORED_EASE }} />; })}
        </svg>
        <p><strong>{values.at(-1)} lb</strong><span>{isLogged ? "logged working load" : "estimated working load"}</span></p>
      </div>
      <p className="training-plot-axis" aria-hidden="true"><span>first working set</span><span>latest working set</span></p>
      <div className="training-series-picker" aria-label="Choose an exercise to preview">
        {TRAINING_SERIES.map((item) => <button key={item.id} type="button" aria-pressed={item.id === activeExercise} onClick={() => setActiveExercise(item.id)}>{item.label}</button>)}
      </div>
    </article>
  );
}
...
function DuolingoCard() {
  const [streak, setStreak] = useState(1223);
  useEffect(() => {
    const controller = new AbortController();
    fetch("/api/duolingo", { signal: controller.signal }).then((response) => response.ok ? response.json() as Promise<{ streak?: number }> : null).then((data) => { if (typeof data?.streak === "number") setStreak(data.streak); }).catch(() => undefined);
    return () => controller.abort();
  }, []);
  return <article className="technical-card technical-card--duolingo" aria-labelledby="duolingo-title"><Image className="technical-image technical-image--duolingo" src="/resume/duolingo-owl.webp" alt="" width={768} height={768} /><div className="technical-card-caption"><h2 id="duolingo-title"><span>{streak.toLocaleString()}</span> day streak</h2><p>Duolingo, every day. The owl is watching.</p></div></article>;
}

function WorkoutCalendar({ workouts }: { workouts: GymWorkout[] | null }) {
  ...
  const workoutPattern = new Set((workouts ?? []).filter((workout) => workout.workoutDate.startsWith(monthPrefix)).map((workout) => Number(workout.workoutDate.slice(-2))));
  const cells = Array.from({ length: firstDay + daysInMonth }, (_, index) => index < firstDay ? null : index - firstDay + 1);
  return (
    <a className="technical-card technical-card--calendar" href={bookingUrl} target="_blank" rel="noreferrer" aria-label="Open Cal.com to book a workout with Kuan Yi Wang">
      <div className="technical-card-heading"><h2>Find a workout</h2><span>Cal.com ↗</span></div>
      <div className="workout-calendar" aria-hidden="true"><p>{date.toLocaleDateString(undefined, { month: "long", year: "numeric" })}</p><div className="workout-calendar-weekdays"><span>M</span><span>T</span><span>W</span><span>T</span><span>F</span><span>S</span><span>S</span></div><div className="workout-calendar-days">{cells.map((day, index) => day === null ? <span key={`blank-${index}`} /> : <span key={day} className={workoutPattern.has(day) ? "is-training" : ""}>{day}</span>)}</div></div>
      <p className="technical-card-description">Your private logged days appear here. Waterloo people: bring a lift, a story, or both.</p>
    </a>
  );
}

function TechnicalCardGrid({ active, reducedMotion }: { active: boolean; reducedMotion: boolean }) {
  const [workouts, setWorkouts] = useState<GymWorkout[] | null>(null);
  useEffect(() => {
    if (!active) return;
    const controller = new AbortController();
    fetch("/api/gym/logs", { signal: controller.signal, cache: "no-store" })
      .then((response) => response.ok ? response.json() as Promise<{ workouts: GymWorkout[] }> : null)
      .then((data) => { if (data?.workouts) setWorkouts(data.workouts); })
      .catch(() => undefined);
    return () => controller.abort();
  }, [active]);
  const reveal = (index: number) => ({ initial: reducedMotion ? false : { opacity: 0, y: 18, clipPath: "inset(100% 0 0 0)" }, animate: { opacity: 1, y: 0, clipPath: "inset(0 0 0 0)" }, exit: { opacity: 0 }, transition: { duration: reducedMotion ? 0 : 0.72, delay: reducedMotion ? 0 : 0.16 + index * 0.12, ease: AUTHORED_EASE } });
  return (
    <AnimatePresence initial={false}>{active ? <motion.aside className="technical-card-grid" aria-label="Technical life notes" initial={reducedMotion ? false : { opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: reducedMotion ? 0 : 0.28, ease: AUTHORED_EASE }}>
      <motion.article className="technical-card technical-card--friends" {...reveal(0)}><Image className="technical-image technical-image--friends" src="/resume/circa-2026-lego.webp" alt="" width={756} height={1008} /><div className="technical-card-caption"><h2>Circa 2026</h2><p>Trolling with friends.</p></div></motion.article>
      <motion.div {...reveal(1)}><DuolingoCard /></motion.div>
      <motion.div {...reveal(2)}><TrainingPlot workouts={workouts} /></motion.div>
      <motion.div {...reveal(3)}><WorkoutCalendar workouts={workouts} /></motion.div>
    </motion.aside> : null}</AnimatePresence>
  );
}

const PROJECT_CARDS = [
  { id: "quantum", title: "GNN Quantum Error Decoder", detail: "Silver Award · ISEF", embed: "https://www.linkedin.com/embed/feed/update/urn:li:ugcPost:7465787183912669184?collapsed=1" },
  { id: "cortesol", title: "Cortesol", detail: "Group Relative Policy Optimization", image: "/resume/project-cortesol.png" },
  { id: "vjepa", title: "Multimodal Conversational Agent", detail: "V-JEPA world model", image: "/resume/project-vjepa.png" },
] as const;

function ProjectCardStack({ active, reducedMotion }: { active: boolean; reducedMotion: boolean }) {
  return (
    <AnimatePresence initial={false}>
      {active ? <motion.aside className="project-card-stack" aria-label="Project details" initial={reducedMotion ? false : { opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: reducedMotion ? 0 : 0.28, ease: AUTHORED_EASE }}>
        {PROJECT_CARDS.map((project, index) => <motion.figure key={project.id} className="project-card" initial={reducedMotion ? false : { opacity: 0, x: -24, clipPath: "inset(0 100% 0 0)" }} animate={{ opacity: 1, x: 0, clipPath: "inset(0 0% 0 0)" }} exit={{ opacity: 0, x: -14, clipPath: "inset(0 100% 0 0)" }} transition={{ duration: reducedMotion ? 0 : 0.82, delay: reducedMotion ? 0 : 0.18 + index * 0.2, ease: AUTHORED_EASE }}>
          <div className={`project-card-image${project.id === "quantum" ? " project-card-image--embed" : ""}`} aria-hidden={project.id === "quantum" ? undefined : true}>
            {project.id === "quantum" ? <iframe src={project.embed} title="Kuan Yi Wang's GNN Quantum Error Decoder LinkedIn post" loading="lazy" allowFullScreen /> : <ImageDithering className="project-card-shader" image={project.image} colorBack="#ebe5da" colorFront="#282521" colorHighlight="#797166" originalColors={false} type="8x8" size={0.72} colorSteps={3} fit="cover" speed={0} minPixelRatio={1} maxPixelCount={120000} />}
          </div>
          <figcaption><strong>{project.title}</strong><span>{project.detail}</span></figcaption>
        </motion.figure>)}
      </motion.aside> : null}
    </AnimatePresence>
  );
...
function MobileResume() {
  return (
    <main className="mobile-resume">
      ...
      <section className="mobile-resume-document" aria-label="Resume preview">
        <Image src="/resume/kuan-yi-wang-resume.svg" alt="Kuan Yi Wang résumé" width={612} height={792} unoptimized />
      </section>
      <section className="mobile-technical-grid" aria-label="Technical life notes">
        <DuolingoCard />
        <article className="technical-card technical-card--friends"><Image className="technical-image technical-image--friends" src="/resume/circa-2026-lego.webp" alt="" width={756} height={1008} /><div className="technical-card-caption"><h2>Circa 2026</h2><p>Trolling with friends.</p></div></article>
        <TrainingPlot workouts={null} />
        <WorkoutCalendar workouts={null} />
      </section>
    </main>
  );
}
      ...
      <CinematicStageContext.Provider value={cinematicStageRef}>
        <div className="portfolio-stage">
          ...
          <DitherDetailCard
            ...
            variant="experience"
          />
          <TechnicalCardGrid active={skillsActive} reducedMotion={reducedMotion} />
